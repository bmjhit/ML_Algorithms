...
def Percetron_Learning(X, y, learning_rate):
    m, n = X.shape
    w_record = []
    X = np.column_stack((X, np.ones(m))) # 矩阵按列合并
    w = np.random.random(n + 1) # n+1列作为参数，w=(w1,w2,b)
    print(w)
...
ani = animation.FuncAnimation(fig, animate, init_func=init, interval=20, blit=True, save_count=150)
